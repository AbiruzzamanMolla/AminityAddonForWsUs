        ...
        <section class="section">
            <div class="section-header">
                ...
                <div class="section-header-breadcrumb">
                    ...
                    <div class="breadcrumb-item active">
                        <a href="{{ route('admin.listing.aminity.index') }}">{{ __('Aminity List') }}</a>
                    </div>
                    <div class="breadcrumb-item">{{ __('Edit Aminity') }} ({{ request('code') }})</div>
                </div>
            </div>
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            ...
                            @adminCan('listing.aminity.translate')
                                <hr>
                                @if ($code !== $languages->first()->code)
                                    <button class="btn btn-primary" id="translate-btn"
                                        onclick="translateAll()">{{ __('Translate') }}</button>
                                @endif
                            @endadminCan
                        </div>
                        <div class="card-body">
                                    ...
                                    @foreach (allLanguages() as $language)
                                        <li>
                                            <a id="{{ request('code') == $language->code ? 'selected-language' : '' }}"
                                                href="{{ route('admin.listing.aminity.edit', ['listing_aminity' => $aminity->id, 'code' => $language->code]) }}">
                                                <i
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header d-flex justify-content-between">
                                <h4>{{ __('Edit Aminity') }}</h4>
                                <div>
                                    <a class="btn btn-primary" href="{{ route('admin.listing.aminity.index') }}"><i
                            ...
                            <div class="card-body">
                                <form
                                    action="{{ route('admin.listing.aminity.update', ['listing_aminity' => $aminity->id, 'code' => $code]) }}"
                                    method="post">
                                    @csrf
                                    @method('PUT')
                                    <div class="row">
                                        <div class="col-md-8 offset-md-2">
                                            <div class="form-group">
                                                ...
                                                <input class="form-control" id="title" name="title"
                                                    ...
                                                    value="{{ old('title', $aminity->getTranslation($code)->title) }}"
